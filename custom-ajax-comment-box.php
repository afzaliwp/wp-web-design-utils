...
// Safe wrapper for WooCommerce functions
function safe_is_shop() {
	return is_woocommerce_active() && function_exists('is_shop') && is_shop();
}

function safe_is_product_category() {
	return is_woocommerce_active() && function_exists('is_product_category') && is_product_category();
}

function safe_is_product_tag() {
	return is_woocommerce_active() && function_exists('is_product_tag') && is_product_tag();
}

function safe_is_singular_product() {
	return is_woocommerce_active() && is_singular('product');
}
...
function handle_woo_page_comment_submission() {
	// Verify nonce
	if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'woo_page_comment_nonce')) {
...
// Custom comment walker (only if WooCommerce walker doesn't exist)
if (!class_exists('WooCommerce_Comment_Walker')) {
	class WooCommerce_Comment_Walker extends Walker_Comment {
		public function start_el(&$output, $comment, $depth = 0, $args = array(), $id = 0) {
			...
			$output .= '<' . $tag . ' id="comment-' . get_comment_ID() . '" class="comment">';
			$output .= '<div class="comment-meta">';
			$output .= '<span class="comment-author">' . get_comment_author() . '</span>';
			$output .= ' - <span class="comment-date">' . get_comment_date() . '</span>';
			$output .= '</div>';
...
function woo_page_comments_shortcode($atts) {
	...
	if ($atts['force_virtual'] === 'no' && (safe_is_singular_product() || is_singular('post') || is_singular('page'))) {
			...
			<div id="comments-display" class="comments-area">
				<?php if ($comments): ?>
					...
					<?php else: ?>
						...
						<ol class="comment-list">
							<?php
							// Use custom walker or default WordPress walker
							$walker = class_exists('WooCommerce_Comment_Walker') ? new WooCommerce_Comment_Walker() : null;
	...
	<?php if ($atts['show_form'] === 'yes' && !$use_native_comments): ?>
	<script>
	...
	<style>
        ...
        /* Additional styles for native WordPress comments */
        .native-comment-form .comment-form-comment textarea,
        .native-comment-form .comment-form-author input,
        .native-comment-form .comment-form-email input,
        .native-comment-form .comment-form-url input {
            direction: rtl;
        }

        .native-comment-form .form-submit input {
            background: #0073aa;
            color: white;
            padding: 12px 24px;
            border: none;
        ...
        .native-comment-form .form-submit input:hover {
            background: #005a87;
        }
	</style>
	<?php
